...
class SerializationTest extends AnyFunSuite with Matchers {
  ...
  test("scala 3 enum field can be serialized and deserialized") {
    ...
    // Use plain deserialize — exercises Avro's automatic MODEL$ discovery via @scala.annotation.static
    deserialize[EnumSpec](serialize(spec), spec.getSchema) shouldBe spec
  }

  test("MODEL$ is a Java static field discoverable via reflection (Scala 3 @static)") {
    val field = classOf[EnumSpec].getDeclaredField("MODEL$")
    field.setAccessible(true)
    val data = field.get(null).asInstanceOf[org.apache.avro.specific.SpecificData]

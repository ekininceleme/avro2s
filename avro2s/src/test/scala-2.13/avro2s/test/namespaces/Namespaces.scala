...
case class Namespaces(var _record_with_explicit_namespace: avro2s.test.namespaces.explicit.RecordWithExplicitNamespace, var _record_with_inherited_namespace: avro2s.test.namespaces.RecordWithInheritedNamespace) extends org.apache.avro.specific.SpecificRecordBase {
  ...
  override def put(field$: Int, value: Any): Unit = {
    (field$: @switch) match {
      case 0 =>
        this._record_with_explicit_namespace = value.asInstanceOf[avro2s.test.namespaces.explicit.RecordWithExplicitNamespace]
      case 1 =>
        this._record_with_inherited_namespace = value.asInstanceOf[avro2s.test.namespaces.RecordWithInheritedNamespace]
      case _ =>
        throw new org.apache.avro.AvroRuntimeException("Bad index")
    }
  }
